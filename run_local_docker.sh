...
# Script to build and run the test 8 MCP Server locally in Docker
...
IMAGE_NAME="test-8-mcp-server"
CONTAINER_NAME="test-8-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running test 8 MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# test 8 MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ test 8 MCP Server is running!${NC}"
echo
