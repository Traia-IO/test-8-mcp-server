#!/usr/bin/env python3
"""
test 8 MCP Server - FastMCP with D402 Transport Wrapper
...
logger = logging.getLogger('test-8_mcp')
...
logger.info("="*80)
logger.info(f"test 8 MCP Server (FastMCP + D402 Wrapper)")
logger.info(f"API: https://petstore.swagger.io/")
logger.info(f"Payment: {SERVER_ADDRESS}")
logger.info("="*80)

# Create FastMCP server
mcp = FastMCP("test 8 MCP Server", host="0.0.0.0")
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="test-8-mcp-server"  # MCP server ID for tracking
    )
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting test 8 MCP Server")
    logger.info("="*80)
